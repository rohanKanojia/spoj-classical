...
public:
  ...
  friend Integer operator-(Integer b, Integer a) {
    ...
    vector<char> tmpStr(b.num, b.num + b.getCapacity());
    if(a > b) {
      Integer tmpInt = b;
      b = a;
      a = tmpInt;
    ...
    for(i = a.getCapacity()-1; i >= 0; i--) {
      if(tmpStr[i]-'0' >= a.getDigit(i)) {
        tmp = (tmpStr[i]-'0') - a.getDigit(i);
      }
      else {
        neighbour = i - 1;
        while(neighbour > 0 && tmpStr[neighbour] == '0') { neighbour--; }

        for(j = neighbour; j < i; j++) {
          tmpStr[j] = tmpStr[j]-1;
          tmpStr[j+1] = tmpStr[j+1] + 10;
        }
        tmp = (tmpStr[i] - '0') - a.getDigit(i);
      }
      C.num[i] = tmp + '0';
...
int main() {
  string ss;
  while(cin >> ss) {
    Integer N(ss), two(string("2")), one("1");
